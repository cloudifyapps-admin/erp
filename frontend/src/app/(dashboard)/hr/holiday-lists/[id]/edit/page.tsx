...
export default function EditHolidayListPage({
  params,
}: {
    ...
    api
      ...
      .then(({ data }) => {
        ...
        if (Array.isArray(data.holidays)) {
          setHolidays(
            data.holidays.map((h: Record<string, unknown>) => ({
      ...
      .catch(() => toast.error('Failed to load holiday list'))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      await api.put(`/hr/holiday-lists/${id}`, {
        name: form.name,
      ...
      const msg =
        (err as { response?: { data?: { detail?: string } } })?.response?.data
          ?.detail ?? 'Failed to update holiday list'
      toast.error(msg)
    } finally {
      setSaving(false)
    }
