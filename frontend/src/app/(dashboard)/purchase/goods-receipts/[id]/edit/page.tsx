...
export default function EditGoodsReceiptPage({
        ...
        if (Array.isArray(data.items) && data.items.length > 0) {
          setItems(
            data.items.map((it: Record<string, unknown>) => ({
              ...
              expected_qty: String(it.expected_qty ?? '0'),
              received_qty: String(it.received_qty ?? '0'),
              unit: String(it.unit ?? 'pcs'),
            }))
          )
        } else {
          setItems([{ id: crypto.randomUUID(), product_name: '', expected_qty: '0', received_qty: '0', unit: 'pcs' }])
        }
      })
  ...
  const setItem = (itemId: string, key: keyof GRItem) => (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      await api.put(`/purchase/goods-receipts/${id}`, {
        ...form,
        items: items
