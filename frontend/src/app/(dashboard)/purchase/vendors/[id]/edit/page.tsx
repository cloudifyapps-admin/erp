...
export default function EditVendorPage({
  ...
  const updateCustomField = (cfId: string, field: 'key' | 'value', val: string) => {
    setCustomFields((prev) =>
      prev.map((cf) => (cf.id === cfId ? { ...cf, [field]: val } : cf))
    )
  }

  const removeCustomField = (cfId: string) => {
    setCustomFields((prev) => prev.filter((cf) => cf.id !== cfId))
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.put(`/purchase/vendors/${id}`, payload)
