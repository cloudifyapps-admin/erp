...
export default function EditStockAdjustmentPage({
        ...
        if (Array.isArray(data.items)) {
          setItems(
            data.items.map((it: Record<string, unknown>) => ({
              ...
              current_qty: String(it.current_qty ?? '0'),
              new_qty: String(it.new_qty ?? '0'),
              notes: String(it.notes ?? ''),
            }))
          )
        }
      })
      .catch(() => toast.error('Failed to load stock adjustment'))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      await api.put(`/inventory/stock-adjustments/${id}`, {
      ...
      const msg =
        (err as { response?: { data?: { detail?: string } } })?.response?.data
          ?.detail ?? 'Failed to update stock adjustment'
      toast.error(msg)
    } finally {
      setSaving(false)
    }
